...
- **Codebase restructured** — `web_bridge.py` split from 7,591-line God Object into 5 domain bridge modules (store, download, game, cloudsaves, misc) + 2,314-line facade delegating all 132 `@Slot` methods. Top-level `sff/` reorganized: `core/` (strings, structs, utils, cache, storage), `network/` (HTTP + Steam protocol), `downloads/` (depot pipeline), `game/` (fix_game, crack_fix, online_fix, steamauto). Empty `home/` and `tools/` directories removed, `manifests/` renamed to `manifest_watcher/`.
